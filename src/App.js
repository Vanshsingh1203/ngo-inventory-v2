...
const T = {
  en: {
    ...
    tutNav:"Navigation",tutNavDesc:"Use the sidebar to navigate between sections. You'll only see tabs relevant to your role.",
    tutReceive:"Receiving Donations",tutReceiveDesc:"Log incoming donations here. Select a category, fill in the details, and submit. Each item gets a unique tracking ID.",
    tutInventory:"Managing Inventory",tutInventoryDesc:"View all items, search and filter, and move items from 'Received' to 'In Storage' by assigning a shelf location.",
    tutDistribute:"Distributing Items",tutDistributeDesc:"Look up items by ID, select quantities to distribute, and record who receives them — individuals or families.",
    tutReports:"Reports & Downloads",tutReportsDesc:"Admins can view year-end summaries and download complete Excel reports with all data.",
    tutEnd:"You're All Set!",tutEndDesc:"You can revisit this tutorial anytime by clicking the help icon in the header. Happy tracking!",
...
function Modal({ open, onClose, children }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16 }} onClick={onClose}>
      <div style={{ background:"#fff",borderRadius:16,maxWidth:480,width:"100%",padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.2)" }} onClick={e=>e.stopPropagation()}>{children}</div>
    </div>
  );
}
...
function Dashboard({ items,monetary,distributions }) {
  ...
  const catData = CATEGORIES.filter(c=>c.code!=="MON").map(c=>({ name:c.name,qty:items.filter(i=>i.cat===c.code).reduce((s,i)=>s+i.qty,0) })).filter(d=>d.qty>0);
  const statusData = Object.entries(items.reduce((a,i)=>{a[i.status]=(a[i.status]||0)+i.qty;return a;},{})).map(([name,value])=>({name,value}));
  const trendData = useMemo(()=>{
    if(view==="monthly"){const d={};items.forEach(i=>{const m=i.date?.substring(0,7);if(m)d[m]=(d[m]||0)+i.qty;});return Object.entries(d).sort().slice(-12).map(([k,v])=>({name:MONTHS[parseInt(k.split("-")[1])-1]+" "+k.split("-")[0],qty:v}));}
    else{const d={};items.forEach(i=>{const y=i.date?.substring(0,4);if(y)d[y]=(d[y]||0)+i.qty;});return Object.entries(d).sort().map(([k,v])=>({name:k,qty:v}));}
  },[items,view]);
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:20 }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(340px, 1fr))",gap:16 }}>
        <div style={{ background:"#fff",borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.05)" }}>
          ...
          <ResponsiveContainer width="100%" height={220}><BarChart data={catData}><XAxis dataKey="name" fontSize={11}/><YAxis fontSize={11}/><Tooltip/><Bar dataKey="qty" fill="#6366f1" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
        </div>
        <div style={{ background:"#fff",borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.05)" }}>
          <h3 style={{ margin:"0 0 16px",fontSize:15,fontWeight:600,color:"#0f172a" }}>{t.statusBreakdown}</h3>
          <ResponsiveContainer width="100%" height={220}><PieChart><Pie data={statusData} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} fontSize={11}>{statusData.map((e,i)=><Cell key={i} fill={STATUS_CLR[e.name]||COLORS[i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
        </div>
      </div>
      <div style={{ background:"#fff",borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.05)" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:12 }}>
          ...
          <div style={{ display:"flex",background:"#f1f5f9",borderRadius:10,padding:3 }}>
            {["monthly","yearly"].map(v=>(<button key={v} onClick={()=>setView(v)} style={{ padding:"6px 16px",borderRadius:8,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",background:view===v?"#fff":"transparent",color:view===v?"#4f46e5":"#64748b",boxShadow:view===v?"0 1px 3px rgba(0,0,0,.1)":"none" }}>{v==="monthly"?t.monthly:t.yearly}</button>))}
          </div>
        </div>
        <ResponsiveContainer width="100%" height={220}><BarChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="name" fontSize={11}/><YAxis fontSize={11}/><Tooltip/><Bar dataKey="qty" fill="#6366f1" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
      </div>
    </div>
  );
}
...
function ReceiveForm({ items,monetary,addItem,addMonetary,showToast }) {
  ...
  const isMon=cat==="MON";const catObj=CATEGORIES.find(c=>c.code===cat);
  ...
  const submit=async()=>{
    ...
    if(isMon){const entry={id:genId("MON",monetary),amount:parseFloat(monAmt),type:monType,donor:donor||"Anonymous",date:new Date().toISOString().split("T")[0],purpose:monPurpose,notes};const ok=await addMonetary(entry);if(ok){setLastEntry({...entry,isMon:true});showToast(lang==="es"?"Donación monetaria registrada":"Monetary donation logged!");}}
    else{const entry={id:genId(cat,items),cat,catName:catObj.name,sub:catObj.subs[subs.indexOf(sub)]||sub,qty:parseInt(qty),condition:cond,donor:donor||"Anonymous",status:"Received",date:new Date().toISOString().split("T")[0],notes,urgent,location:"",created_by:profile?.id};const ok=await addItem(entry);if(ok){setLastEntry(entry);showToast(lang==="es"?`${catName} recibido`:`${catName} — ${sub} (x${qty}) received!`);}}
    ...
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(360px, 1fr))",gap:20}}>
        <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div><label style={lbl}>{t.category} *</label><select value={cat} onChange={e=>{setCat(e.target.value);setSub("");}} style={inp}><option value="">— {t.selectCategory} —</option>{CATEGORIES.map(c=><option key={c.code} value={c.code}>{c.code} — {lang==="es"?c.nameEs:c.name}</option>)}</select></div>
            {cat&&!isMon&&(<><div><label style={lbl}>{t.subcategory} *</label><select value={sub} onChange={e=>setSub(e.target.value)} style={inp}><option value="">— {t.selectSub} —</option>{subs.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div><label style={lbl}>{t.quantity} *</label><input type="number" min="1" value={qty} onChange={e=>setQty(e.target.value)} placeholder="25" style={inp}/></div><div><label style={lbl}>{t.condition}</label><select value={cond} onChange={e=>setCond(e.target.value)} style={inp}>{CONDITIONS.map(c=><option key={c}>{c}</option>)}</select></div></div></>)}
            ...
            {cat&&(<><div><label style={lbl}>{t.donorName}</label><input value={donor} onChange={e=>setDonor(e.target.value)} placeholder={t.anonymous} style={inp}/></div>
              ...
              {!isMon&&<label style={{display:"flex",alignItems:"center",gap:8,fontSize:13,cursor:"pointer",color:urgent?"#e11d48":"#64748b",fontWeight:500}}><input type="checkbox" checked={urgent} onChange={e=>setUrgent(e.target.checked)} style={{accentColor:"#e11d48"}}/> {t.markUrgent}</label>}
        ...
        <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
          ...
          {lastEntry?(<div style={{background:"#f8fafc",borderRadius:12,padding:20}}><div style={{fontSize:13,color:"#475569",lineHeight:2.2}}>
            ...
            <b>{t.donor}:</b> {lastEntry.donor}<br/><b>{t.date}:</b> {lastEntry.date}</div></div>):(<div style={{textAlign:"center",color:"#94a3b8",padding:48,fontSize:14}}><ClipboardList size={40} style={{marginBottom:12,opacity:.4}}/><br/>{t.submitEntry}</div>)}
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <div style={{background:"#fff",borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
          <div style={{flex:"1 1 220px",position:"relative"}}><Search size={16} style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#94a3b8"}}/><input placeholder={t.searchPlaceholder} value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,width:"100%",paddingLeft:36,boxSizing:"border-box"}}/></div>
          <select value={fCat} onChange={e=>setFCat(e.target.value)} style={inp}><option value="">{t.allCategories}</option>{CATEGORIES.filter(c=>c.code!=="MON").map(c=><option key={c.code} value={c.code}>{c.name}</option>)}</select>
          <select value={fStatus} onChange={e=>setFStatus(e.target.value)} style={inp}><option value="">{t.allStatuses}</option><option value="Received">{t.received}</option><option value="In Storage">{t.inStorageStatus}</option><option value="Distributed">{t.distributed}</option></select>
        </div>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:"#f8fafc",borderBottom:"2px solid #e2e8f0"}}>{[t.id,t.category,t.subcategory,t.qty,t.condition,t.donor,t.status,t.location,t.date,t.action].map(h=><th key={h} style={{padding:"12px 8px",textAlign:"left",color:"#475569",fontWeight:600,whiteSpace:"nowrap",fontSize:12}}>{h}</th>)}</tr></thead>
            <tbody>{filtered.map(i=>(
              <tr key={i.id} style={{borderBottom:"1px solid #f1f5f9",background:i.urgent&&i.status!=="Distributed"?"#fef2f2":"transparent"}}>
                <td style={{padding:"12px 8px",fontFamily:"monospace",fontSize:11}}>{i.id}</td><td style={{padding:"12px 8px"}}>{i.catName}</td><td style={{padding:"12px 8px"}}>{i.sub}</td>
                <td style={{padding:"12px 8px",fontWeight:700}}>{i.qty}</td><td style={{padding:"12px 8px"}}>{i.condition}</td><td style={{padding:"12px 8px"}}>{i.donor}</td>
                ...
                <td style={{padding:"12px 8px",fontSize:12,color:"#64748b"}}>{i.location||"—"}</td><td style={{padding:"12px 8px",fontSize:12,color:"#64748b"}}>{i.date}</td>
                <td style={{padding:"12px 8px",whiteSpace:"nowrap"}}>
                  {i.status==="Received"&&(<div style={{display:"flex",gap:6,alignItems:"center"}}><input placeholder={t.enterLocation} value={locInput[i.id]||""} onChange={e=>setLocInput(p=>({...p,[i.id]:e.target.value}))} style={{padding:"6px 10px",border:"1px solid #d1d5db",borderRadius:8,fontSize:12,width:160}}/><button onClick={()=>moveToStorage(i.id)} style={{padding:"6px 12px",background:"#4f46e5",color:"#fff",border:"none",borderRadius:8,fontSize:11,cursor:"pointer",fontWeight:600,display:"flex",alignItems:"center",gap:4}}><ArrowRight size={14}/></button></div>)}
...
function DistributeView({ items,addItem,updateItem,addDistribution,showToast }) {
  const { t,lang,profile } = useApp();
  const [scanId,setScanId]=useState("");const [distQty,setDistQty]=useState("");const [found,setFound]=useState(null);
  const [recipientType,setRecipientType]=useState("individual");const [familySize,setFamilySize]=useState("");const [showConfirm,setShowConfirm]=useState(false);
  const lookup=()=>{const item=items.find(i=>i.id.toLowerCase()===scanId.toLowerCase()&&i.status==="In Storage");setFound(item||"not_found");};
  const distribute=async()=>{
    if(!found||found==="not_found")return;setShowConfirm(false);
    const q=parseInt(distQty)||found.qty;const ppl=recipientType==="family"?parseInt(familySize)||1:1;
    if(q>=found.qty){await updateItem(found.id,{status:"Distributed",location:""});}
    else{await updateItem(found.id,{qty:found.qty-q});await addItem({id:found.id+"-D"+Date.now(),cat:found.cat,catName:found.catName,sub:found.sub,qty:q,condition:found.condition,donor:found.donor,status:"Distributed",date:new Date().toISOString().split("T")[0],notes:`Partial from ${found.id}`,urgent:false,location:""});}
  ...
  return (
    <>
      <Modal open={showConfirm} onClose={()=>setShowConfirm(false)}>
        <div style={{textAlign:"center",marginBottom:20}}><div style={{width:48,height:48,borderRadius:12,background:"#ecfdf5",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",color:"#10b981"}}><Truck size={24}/></div><h3 style={{margin:0,fontSize:18,fontWeight:700}}>{t.confirmSubmit}</h3></div>
        <div style={{background:"#f8fafc",borderRadius:10,padding:16,fontSize:13,color:"#475569",lineHeight:2,marginBottom:20}}><b>{t.category}:</b> {found?.catName} → {found?.sub}<br/><b>{t.qty}:</b> {distQty||found?.qty}<br/><b>{t.recipientType}:</b> {recipientType==="family"?t.family:t.individual}<br/>{recipientType==="family"&&<><b>{t.peopleCount}:</b> {familySize||1}<br/></>}</div>
        <div style={{display:"flex",gap:12}}><button onClick={()=>setShowConfirm(false)} style={{flex:1,padding:"11px",background:"#f1f5f9",color:"#475569",border:"none",borderRadius:10,fontSize:14,fontWeight:600,cursor:"pointer"}}>{t.cancel}</button><button onClick={distribute} style={{flex:1,padding:"11px",background:"#10b981",color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:600,cursor:"pointer"}}>{t.confirm}</button></div>
      </Modal>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(360px, 1fr))",gap:20}}>
        <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
          ...
          {found&&found!=="not_found"&&(
            <div style={{border:"1px solid #d1fae5",borderRadius:12,padding:20,background:"#f0fdf4"}}>
              ...
              <div style={{fontSize:13,color:"#475569",lineHeight:1.8,marginBottom:16}}><b>{found.catName}</b> → {found.sub}<br/>{t.qtyAvailable}: <b>{found.qty}</b> | {t.location}: {found.location}</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div><label style={{fontSize:13,fontWeight:600,color:"#374151",display:"block",marginBottom:8}}>{t.recipientType}</label>
                  <div style={{display:"flex",gap:8}}>{["individual","family"].map(rt=>(<button key={rt} onClick={()=>setRecipientType(rt)} style={{flex:1,padding:"10px",borderRadius:10,border:recipientType===rt?"2px solid #4f46e5":"1px solid #d1d5db",background:recipientType===rt?"#eef2ff":"#fff",color:recipientType===rt?"#4f46e5":"#64748b",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>{rt==="individual"?<User size={16}/>:<Users size={16}/>} {rt==="individual"?t.individual:t.family}</button>))}</div>
                </div>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
          ...
          <div style={{maxHeight:460,overflowY:"auto"}}>{available.map(i=>(<div key={i.id} onClick={()=>{setScanId(i.id);setFound(i);}} style={{padding:"14px 16px",borderBottom:"1px solid #f1f5f9",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13}}><div><b>{i.catName}</b> — {i.sub}<div style={{fontSize:11,color:"#94a3b8",fontFamily:"monospace",marginTop:2}}>{i.id}</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:700}}>×{i.qty}</div><div style={{fontSize:11,color:"#94a3b8"}}>{i.location}</div></div></div>))}{available.length===0&&<div style={{textAlign:"center",padding:48,color:"#94a3b8"}}>{t.noItemsStorage}</div>}</div>
        </div>
      </div>
    </>
  );
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(360px, 1fr))",gap:20}}>
      <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#4f46e5,#7c3aed)",borderRadius:14,padding:24,color:"#fff",marginBottom:20}}><div style={{fontSize:13,opacity:.8}}>{t.totalMonetary}</div><div style={{fontSize:36,fontWeight:700,marginTop:4}}>${total.toLocaleString()}</div><div style={{fontSize:13,opacity:.8,marginTop:4}}>{monetary.length} {t.transactions}</div></div>
        {monetary.map(m=>(<div key={m.id} style={{padding:"14px 0",borderBottom:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between",fontSize:13}}><div><b>${m.amount.toLocaleString()}</b> — {m.type}<br/><span style={{fontSize:12,color:"#94a3b8"}}>{m.donor} · {m.purpose}</span></div><div style={{fontSize:12,color:"#94a3b8"}}>{m.date}</div></div>))}
      </div>
      <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
        <h3 style={{margin:"0 0 20px",fontSize:15,fontWeight:600,color:"#0f172a"}}>{t.byPaymentType}</h3>
        <ResponsiveContainer width="100%" height={280}><PieChart><Pie data={typeData} cx="50%" cy="50%" outerRadius={100} dataKey="value" label={({name,value})=>`${name}: $${value}`} fontSize={12}>{typeData.map((_,i)=><Cell key={i} fill={COLORS[i]}/>)}</Pie><Tooltip formatter={v=>`$${v}`}/></PieChart></ResponsiveContainer>
      </div>
    </div>
  );
}
...
function ReportsView({ items,monetary,distributions }) {
  const { t } = useApp();
  const totalRec=items.reduce((s,i)=>s+i.qty,0);const totalDist=items.filter(i=>i.status==="Distributed").reduce((s,i)=>s+i.qty,0);
  const totalStock=items.filter(i=>i.status==="In Storage").reduce((s,i)=>s+i.qty,0);const totalMoney=monetary.reduce((s,m)=>s+m.amount,0);
  ...
  const catBreakdown=CATEGORIES.filter(c=>c.code!=="MON").map(c=>{const ci=items.filter(i=>i.cat===c.code);return{name:c.name,received:ci.reduce((s,i)=>s+i.qty,0),distributed:ci.filter(i=>i.status==="Distributed").reduce((s,i)=>s+i.qty,0)};}).filter(d=>d.received>0);
  const downloadExcel=()=>{
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(items.map(i=>({ID:i.id,Category:i.catName,Subcategory:i.sub,Quantity:i.qty,Condition:i.condition,Donor:i.donor,Status:i.status,Location:i.location,Date:i.date,Notes:i.notes,Urgent:i.urgent?"Yes":"No"}))),"Items");
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(monetary.map(m=>({ID:m.id,Amount:m.amount,Type:m.type,Donor:m.donor,Date:m.date,Purpose:m.purpose,Notes:m.notes}))),"Monetary");
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet([{Metric:"Total Items Received",Value:totalRec},{Metric:"Total Distributed",Value:totalDist},{Metric:"In Stock",Value:totalStock},{Metric:"Monetary Total",Value:`$${totalMoney}`},{Metric:"Unique Donors",Value:donors},{Metric:"People Served",Value:pplServed},{Metric:"Families Served",Value:families},{Metric:"Distribution Rate",Value:totalRec?`${Math.round(totalDist/totalRec*100)}%`:"0%"}]),"Summary");
  ...
  return (
    <div style={{background:"#fff",borderRadius:14,padding:28,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,marginBottom:24}}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center"}}><span style={{fontSize:12,color:"#94a3b8"}}>{t.generated}: {new Date().toLocaleDateString()}</span><button onClick={downloadExcel} style={{padding:"9px 18px",background:"#4f46e5",color:"#fff",border:"none",borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}><Download size={16}/> {t.downloadExcel}</button></div>
      </div>
      <div style={{background:"linear-gradient(135deg,#0f172a,#1e293b)",borderRadius:14,padding:28,color:"#fff",marginBottom:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(120px, 1fr))",gap:12}}>{stats.map((s,i)=><div key={i} style={{background:"rgba(255,255,255,.08)",borderRadius:10,padding:"14px 12px"}}><div style={{fontSize:22,fontWeight:700}}>{s.v}</div><div style={{fontSize:11,opacity:.7,marginTop:4}}>{s.l}</div></div>)}</div>
      </div>
      <h3 style={{fontSize:15,fontWeight:600,color:"#0f172a",margin:"0 0 14px"}}>{t.catBreakdown}</h3>
      <ResponsiveContainer width="100%" height={260}><BarChart data={catBreakdown}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="name" fontSize={11}/><YAxis fontSize={11}/><Tooltip/><Legend/><Bar dataKey="received" fill="#6366f1" name={t.received} radius={[6,6,0,0]}/><Bar dataKey="distributed" fill="#10b981" name={t.distributed} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
...
export default function App() {
  ...
  const t=T[lang];const showToast=(msg)=>{setToast(msg);setTimeout(()=>setToast(null),2500);};
  ...
  useEffect(()=>{if(!session?.user){setProfile(null);setLoading(false);return;}const loadProfile=async()=>{const{data}=await supabase.from("profiles").select("*").eq("id",session.user.id).single();if(data){setProfile(data);setLang(data.language||"en");if(!data.tutorial_completed)setShowTutorial(true);}setLoading(false);};loadProfile();},[session]);
  ...
  const fetchData=useCallback(async()=>{if(!session)return;const[iR,mR,dR]=await Promise.all([supabase.from("items").select("*").eq("deleted",false).order("created_at",{ascending:false}),supabase.from("monetary").select("*").eq("deleted",false).order("created_at",{ascending:false}),supabase.from("distributions").select("*").order("created_at",{ascending:false})]);
    ...
    setDistributions(dR.data||[]);},[session]);
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"#f8fafc",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif"}}>
        <div style={{width:sidebarOpen?240:0,minWidth:sidebarOpen?240:0,background:"#0f172a",transition:"all .3s",overflow:"hidden",position:"fixed",top:0,left:0,bottom:0,zIndex:900,display:"flex",flexDirection:"column"}}>
          ...
          <div style={{padding:"16px 14px",borderTop:"1px solid rgba(255,255,255,.08)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><div style={{width:34,height:34,borderRadius:10,background:roleColor+"20",display:"flex",alignItems:"center",justifyContent:"center",color:roleColor}}><Shield size={16}/></div><div><div style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>{profile?.full_name}</div><div style={{fontSize:11,color:roleColor,fontWeight:600}}>{roleLabel}</div></div></div>
        ...
        <div style={{flex:1,marginLeft:0,minWidth:0}}>
          <header style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}><button onClick={()=>setSidebarOpen(!sidebarOpen)} style={{background:"none",border:"none",cursor:"pointer",color:"#475569",padding:4}}><Menu size={22}/></button><h2 style={{margin:0,fontSize:17,fontWeight:700,color:"#0f172a"}}>{navItems.find(n=>n.id===page)?.label}</h2></div>
            <div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={toggleLang} style={{padding:"6px 12px",background:"#f1f5f9",border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",color:"#475569",display:"flex",alignItems:"center",gap:4}}><Globe size={15}/> {lang.toUpperCase()}</button><button onClick={()=>setShowTutorial(true)} style={{padding:"6px 10px",background:"#f1f5f9",border:"none",borderRadius:8,cursor:"pointer",color:"#475569",display:"flex",alignItems:"center",gap:4}}><HelpCircle size={16}/></button></div>
          </header>
